...
To contribute to build-tools project on [GitHub](https://github.com/hadenlabs/build-tools), We use [GitHub Flow](https://githubflow.github.io/), it means that you should branch from the main repository and contribute back by making [pull request](https://github.com/hadenlabs/build-tools/pulls).
...
To follow the instructions in this guide and start contributing to build-tools project on GitHub:
...
```bash
git remote add upstream  https://github.com/hadenlabs/build-tools.git
```

Now your **upstream** points to **hadenlabs/build-tools**.
...
git fetch upstream # assuming upstream is hadenlabs/build-tools
...
All merge requests are automatically tested using [Github Actions](https://github.com/hadenlabs/build-tools/actions). In case some tests fail, fix the issues or describe why the fix cannot be done.
